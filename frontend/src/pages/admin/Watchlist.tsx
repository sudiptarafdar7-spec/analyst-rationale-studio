...
import {
  CalendarClock, Check, Loader2, RefreshCw, Search, Target, Trash2, TrendingUp, X,
} from "lucide-react";
...
interface Row {
  ...
  holding_period: string | null; holding_period_days: number | null; analysis_text: string | null;
          ...
          <table className="w-full min-w-[1180px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-left text-xs uppercase tracking-wide text-slate-400">
                ...
                <th className="px-3 py-2.5 text-right font-semibold">CMP @ call</th>
                <th className="px-3 py-2.5 font-semibold">Targets</th>
                <th className="px-3 py-2.5 text-right font-semibold">SL</th>
                <th className="px-3 py-2.5 text-right font-semibold">Downfall</th>
                <th className="px-3 py-2.5 font-semibold">Holding</th>
                ...
                {rows.map((r, idx) => {
                  ...
                  return (
                    <motion.tr key={r.id}
                      ...
                      className={`border-b border-slate-100 align-top ${ROW_BG[r.highlight] ?? "bg-white"} hover:brightness-[0.99]`}>
                      <td className="whitespace-nowrap px-3 py-3 text-slate-600">
                        <div className="flex items-center gap-1 text-xs"><CalendarClock size={12} className="text-slate-400" />{r.call_date ?? "—"}</div>
                        <div className="mt-0.5 text-[11px] text-slate-400">{daysLabel(r.days_since)}</div>
                      </td>
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-2">
                          {r.channel_logo_path
                            ? <img src={r.channel_logo_path} alt="" className="h-7 w-7 shrink-0 rounded-md object-cover ring-1 ring-slate-200" />
                            : <span className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-slate-100 text-[10px] font-semibold text-slate-500">{(r.platform_name ?? "?")[0]?.toUpperCase()}</span>}
                          <div className="min-w-0">
                            <div className="truncate text-xs font-medium text-slate-700">{r.platform_name ?? "—"}</div>
                            <div className="truncate text-[11px] capitalize text-slate-400">{r.platform_type ?? ""}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-3 py-3">
                        <div className="font-semibold text-slate-800">{r.stock_symbol ?? r.short_name ?? "—"}</div>
                        <div className="max-w-[170px] truncate text-[11px] text-slate-400" title={r.listed_name ?? ""}>{r.listed_name ?? ""}</div>
                      </td>
                      <td className="px-3 py-3"><span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${cm.cls}`}>{cm.label}</span></td>
                      <td className="whitespace-nowrap px-3 py-3 text-right text-slate-700">{inr(r.call_cmp)}</td>
                      <td className="px-3 py-3">
                        {r.targets_status.length === 0 ? <span className="text-xs text-slate-300">—</span> : (
                          <div className="flex flex-wrap gap-1">
                            {r.targets_status.map((t, i) => (
                              <span key={i} title={`Target ${i + 1}`}
                                className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[11px] font-medium ${t.achieved ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
                                T{i + 1} {t.achieved && <Check size={11} />} {inr(t.value)}
                              </span>
                            ))}
                          </div>
                        )}
                      </td>
                      <td className="whitespace-nowrap px-3 py-3 text-right text-slate-600">{inr(r.stoploss)}</td>
                      <td className="whitespace-nowrap px-3 py-3 text-right text-slate-600">{r.downfall_target == null ? "—" : (
                        <span className={r.downfall_hit ? "font-semibold text-red-600" : ""}>{inr(r.downfall_target)}</span>
                      )}</td>
                      ...
                      <td className="whitespace-nowrap px-3 py-3 text-right">
                        {r.current_cmp == null ? <span className="text-xs text-slate-300">not fetched</span> : <span className="font-medium text-slate-800">{inr(r.current_cmp)}</span>}
                      </td>
                      <td className="whitespace-nowrap px-3 py-3 text-right">
                        {!showPnl ? <span className="text-xs text-slate-300">—</span> : (
                          <span className={`font-semibold ${r.pnl_abs! >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                            {r.pnl_abs! >= 0 ? "+" : ""}{r.pnl_pct}%<div className="text-[11px] font-normal">{r.pnl_abs! >= 0 ? "+" : ""}{inr(r.pnl_abs)}</div>
                          </span>
                        )}
                      </td>
                      <td className="px-3 py-3">
                        {r.status === "achieved" ? (
                          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-700"><Check size={12} />{r.achieved_count}/{r.total_targets || 1} hit</span>
                        ) : (
                          <span className="inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-500">Awaited</span>
                        )}
                      </td>
                      <td className="whitespace-nowrap px-3 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button title="Fetch CMP" disabled={refreshOne.isPending} onClick={() => refreshOne.mutate(r.id)}
                            ...
                            {refreshOne.isPending && refreshOne.variables === r.id ? <Loader2 size={14} className="animate-spin" /> : <RefreshCw size={14} />}
                          </button>
                          <button title="Remove from watchlist" disabled={remove.isPending} onClick={() => remove.mutate(r.id)}
                            className="grid h-7 w-7 place-items-center rounded-lg text-slate-400 transition hover:bg-white hover:text-danger"><Trash2 size={14} /></button>
                        </div>
                      </td>
                    </motion.tr>
                  );
